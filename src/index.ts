...
// Ángulos cinemáticos
let angleBase: number = 0;
let angleShoulder: number = 0;
let angleElbow: number = 0;
let angleGripper: number = 0;
...
function vp(dTheta: number, dPhi: number, fRho: number): void {  // Vista/Cámara
  if (obj != undefined) {
...
function applyKinematics() {
  ...
  if (obj.wOrg[301] !== undefined && obj.wOrg[300] !== undefined && obj.wOrg[319] !== undefined) {
    ...
    for (let i = 301; i <= 312; i++) {
      if (obj.w[i]) obj.w[i] = Rota3D.rotate(obj.w[i]);
    }
  }
  ...
  if (obj.wOrg[29] !== undefined && obj.wOrg[30] !== undefined) {
    ...
    if (obj.w[139]) obj.w[139] = Rota3D.rotate(obj.w[139]);
    if (obj.w[140]) obj.w[140] = Rota3D.rotate(obj.w[140]);
  ...
  if (obj.w[29]) obj.w[29] = Rota3D.rotate(obj.w[29]);
  if (obj.w[30]) obj.w[30] = Rota3D.rotate(obj.w[30]);
...
function resetAngles() {
  ...
  let sliderBase = <HTMLInputElement>document.getElementById('sliderBase');
  let sliderShoulder = <HTMLInputElement>document.getElementById('sliderShoulder');
  let sliderElbow = <HTMLInputElement>document.getElementById('sliderElbow');
  let sliderGripper = <HTMLInputElement>document.getElementById('sliderGripper');

  if (sliderBase) sliderBase.value = "0";
  if (sliderShoulder) sliderShoulder.value = "0";
  if (sliderElbow) sliderElbow.value = "0";
  if (sliderGripper) sliderGripper.value = "0";
  ...
  if (valBase) valBase.innerText = "0°";
  if (valShoulder) valShoulder.innerText = "0°";
  if (valElbow) valElbow.innerText = "0°";
  if (valGripper) valGripper.innerText = "0°";
}

function actualizarVisibilidadControles(fileName: string) {
...
function cargarModeloPredeterminado(fileName: string) {
  let contenidoArchivo = document.getElementById('contenido-archivo');
  if (contenidoArchivo) contenidoArchivo.innerText = "Cargando modelo " + fileName + "...";
  ...
  fetch('./' + fileName)
    ...
    .catch(error => {
      console.error(error);
      if (contenidoArchivo) contenidoArchivo.innerText = "Error al cargar modelo:\n" + error.message;
    });
}

// Vinculación de eventos a los sliders
document.getElementById('sliderBase').addEventListener('input', (e: any) => {
  angleBase = Number(e.target.value);
  document.getElementById('valBase').innerText = angleBase + "°";
  applyKinematics();
});

document.getElementById('sliderShoulder').addEventListener('input', (e: any) => {
  angleShoulder = Number(e.target.value);
  document.getElementById('valShoulder').innerText = angleShoulder + "°";
  applyKinematics();
});

document.getElementById('sliderElbow').addEventListener('input', (e: any) => {
  angleElbow = Number(e.target.value);
  document.getElementById('valElbow').innerText = angleElbow + "°";
  applyKinematics();
});

document.getElementById('sliderGripper').addEventListener('input', (e: any) => {
...
function makeVisualisation(evento: any) {
  if (flag) {
    Pfx = evento.offsetX;
    Pfy = evento.offsetY;
    let difX = Pix - Pfx;
